...
const categoryData: Array<ICategory> = [
  {
    id: 1,
    name: 'Rice',
...
export const Home: React.FC<NativeStackScreenProps<RootStackParamList>> = ({
  navigation,
  route,
}) => {
  ...
  const handleCategorySelect = (category: React.SetStateAction<any>) => {
  ...
  const renderCategories = () => {
    const renderItem = ({
      item,
    }: ListRenderItemInfo<{id: number; name: string; icon: any}>) => {
      return (
        <TouchableOpacity
  ...
  const renderRestaurants = () => {
    const renderItem = (item: IRestaurant) => {
      return (
        <TouchableOpacity
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <RenderHeader stressName={currentLocation.streetName} />
      {renderCategories()}
      {renderRestaurants()}
    </SafeAreaView>
  );
...
const RenderHeader: React.FC<RenderHeaderProps> = ({stressName}) => {
  return (
    <View
      style={
        Platform.OS === 'android'
      ...
      <View
        ...
        <View
          ...
          <Text style={{...FONTS.h3, color: 'black'}}>{stressName}</Text>
        </View>
      </View>
      <TouchableOpacity
        style={{
